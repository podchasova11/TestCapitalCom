...
class SellButtonTableMostTraded(BasePage):
    ...
    @allure.step("Click button BUTTON_TRADING_SELL_MOST_TRADED")
    def element_click(self, cur_item_link, cur_language, cur_role):
        ...
        if len(button_list) >= 1:
            ...
            print(f"{datetime.now()}   => BUTTON_TRADING_SELL_MOST_TRADED is not present on the page!")
            del button_list
    ...
    def ClickButton(self, times, cur_item_link, cur_language, cur_role):
        for i in range(times):
            ...
            print(f"{datetime.now()}   Is BUTTON_TRADING_SELL_MOST_TRADED_#{i + 1} clickable? =>")
            ...
            try:
                ...
                button_list[i].click()
                print(f"{datetime.now()}   => BUTTON_TRADING_SELL_MOST_TRADED_#{i + 1} clicked!")
